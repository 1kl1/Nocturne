...
> 선택한 Notion 페이지와 하위 페이지를 매일 밤 agent가 점검해 사실 오류/누락/모순을 찾아내고, 사용자가 승인한 제안만 원문에 부분 반영하는 web application MVP
...
Nocturne MVP는 Notion 사용자가 연결한 페이지를 매일 밤 자동 점검하고, 수정 제안을 Notion 수정함에 쌓은 뒤, Slack 또는 이메일로 아침 알림을 보내고, 사용자가 승인한 항목만 agent가 원문에 반영하는 서비스다.
...
- Web application 기반 온보딩 및 설정 화면
- Notion OAuth 연결
- OpenRouter API 키 등록
...
| Agent 범위 | 최근 수정 페이지 검사부터 승인 항목 원문 반영까지 수행. 단, 원문 수정은 승인 항목만 |
...
핵심 원칙은 "agent가 실행하되, 사용자가 승인하기 전에는 원본을 수정하지 않는다"이다. agent는 수집, 분석, 제안 저장, 알림, 승인 항목 반영까지 수행하지만, 원본 Notion 페이지에 대한 쓰기 작업은 승인 상태인 레코드에만 제한된다.
...
- 이메일 주소 입력 시 인증 코드를 발송하고 사용자가 코드를 입력하면 연결 완료 처리한다.
...
- Slack과 이메일이 모두 연결되어 있으면 둘 다 발송할 수 있다.
- 둘 중 하나만 연결되어 있으면 해당 채널로만 발송한다.

### 3.4 실행 로그 화면

사용자는 agent 실행 내역을 확인할 수 있다.
...
agent harness는 야간 점검과 승인 반영을 수행하는 실행 엔진이다. 별도 Hermes agent 구현체는 없으므로, Hermes식 agent 구조인 계획, 도구 호출, 검증, 기록, 재시도 패턴을 참고해 구현한다.
...
| Web Search Tool Adapter | 전체 웹 검색 수행 및 출처 URL 수집 |
...
- 원본 Notion 페이지 수정은 `승인` 상태의 제안에만 수행한다.
...
사실 오류 검증에는 전체 웹 검색을 사용할 수 있다.

검색 원칙:

- 검증 가능한 주장에 대해서만 검색한다.
- 검색 결과 URL, 제목, 요약, 접근 시각을 저장한다.
- 출처가 불명확하거나 신뢰하기 어려우면 확신도를 낮춘다.
- 검색으로 확인할 수 없는 항목은 단정하지 않는다.
- 웹 검색이 실패해도 누락/모순 분석은 계속한다.

MVP 구현에서는 web search provider를 adapter로 감싼다. 실제 provider는 환경변수로 설정할 수 있게 하되, agent 내부에서는 `search_web(query)` 인터페이스만 사용한다.

### 6.3 LLM 호출

LLM은 OpenRouter API를 통해 호출한다.
...
야간 run에서는 이전 실행에서 생성되어 사용자가 이미 승인한 항목을 먼저 반영한 뒤, 새로운 최근 수정 페이지 검사를 수행한다. 이렇게 하면 사용자가 아침 알림을 보고 낮에 승인한 제안이 다음 야간 실행에서 자동 반영된다.

### 8.2 반영 규칙

agent는 `상태=승인`인 수정함 레코드만 처리한다.
...
사용자는 둘 중 하나만 연결해도 되고, 둘 다 연결할 수 있다.
...
| 백엔드 | Python, FastAPI |
| 스케줄러 | APScheduler 또는 cron 기반 내부 스케줄러 |
...
- Git repository 또는 Docker image 기반 배포
...
Notion 워크스페이스 안에 생성된 수정함 DB 삭제는 사용자가 직접 할 수 있게 안내한다. MVP에서는 외부 워크스페이스 데이터 자동 삭제를 기본 동작으로 삼지 않는다.
...
- Slack 실패 후 이메일이 연결되어 있으면 이메일 발송 계속
- 이메일 실패 후 Slack이 연결되어 있으면 Slack 발송 계속
...
- 사용자는 web application에서 Notion OAuth 연결을 완료할 수 있다.
- 사용자는 OpenRouter API 키를 등록하고 유효성 확인을 받을 수 있다.
- 사용자는 Slack webhook 또는 이메일 주소 중 하나 이상을 연결할 수 있다.

### 16.2 점검 대상

- 사용자는 Notion 페이지 또는 데이터베이스를 점검 대상으로 추가할 수 있다.
...
- Coolify에서 환경변수와 volume을 설정해 배포할 수 있다.
...
- agent는 최근 수정 페이지 검사, 분석, 제안 저장, 알림, 승인 항목 원문 반영까지 수행하지만, 원문 수정은 승인 항목에만 제한한다.
